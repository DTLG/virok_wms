...
void showPlacementNomScanDialog(BuildContext context, AdmissionNom nom) {
  showDialog(
      ...
      builder: (_) => BlocProvider.value(
            value: context.read<PlacementFromReturnCubit>(),
            child: NomScanDialog(
              nom: nom,
            ),
          ));
...
class _NomScanDialogState extends State<NomScanDialog> {
  @override
  void initState() {
    context.read<PlacementFromReturnCubit>().getNom(
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        await context.read<PlacementFromReturnCubit>().clear();
        return true;
      },
      child: AlertDialog(
        ...
        icon: DialogHead(
          ...
          onPressed: () {
            context.read<PlacementFromReturnCubit>().clear();
        ...
        content: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            BlocBuilder<PlacementFromReturnCubit, PlacementFromRetrunState>(
...
class _CellInputState extends State<CellInput> {
  ...
  Widget build(BuildContext context) {
    final cellValue =
        context.select((PlacementFromReturnCubit cubit) => cubit.state.cell);
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: TextField(
        ...
        focusNode: focusNode,
        onSubmitted: (value) {
          context.read<PlacementFromReturnCubit>().scan(value, widget.nom);
          controller.clear();
          focusNode.requestFocus();
        },
        decoration: InputDecoration(
...
class ManualInputButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<PlacementFromReturnCubit, PlacementFromRetrunState>(
      builder: (context, state) {
        return ElevatedButton(
            style: ButtonStyle(
                backgroundColor: MaterialStatePropertyAll(
                    context.read<PlacementFromReturnCubit>().state.count > 0
                        ? Colors.green
                        : Colors.grey)),
            onPressed: () {
        ...
        actions: [
          Column(
            children: [
              ...
              ElevatedButton(
                  onPressed: () {
                    context.read<PlacementFromReturnCubit>().cancelTask(nom);
                    Navigator.pop(context);
                    Navigator.pop(context);
                  },
                  style: ButtonStyle(
