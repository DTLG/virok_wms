...
import 'package:virok_wms/feature/recharge/recharge_repository/recharge_repository.dart';
...
class RechargeCubit extends Cubit<RechargeState> {
  ...
  void scanNomWritingOff(String scanBarcode, RechargeNom nom) {
    int count = state.count == 0 ? nom.countTake : state.count;
    String checkNomBar = '';
  ...
  void scanNomPlacement(String scanBarcode, RechargeNom nom) {
    int count = state.count == 0 ? nom.countPut : state.count;
    String checkNomBar = '';
    for (var barcode in nom.barcodes) {
  ...
  clear() {
    emit(state.copyWith(
        status: RechargeStatus.success, errorMassage: '', cell: '', count: 0, nomBarcode: ''));
  }
}
